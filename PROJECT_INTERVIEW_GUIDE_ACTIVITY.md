...
- Automation SDK (Java 17+) — independent
- krista-whatsapp (Java 21) — ACTIVE rewrite target
- heloai-waba-extension — OLD, replaced
- heloai-waba-persistence — OLD, replaced
- krista-extension-skills — documentation & guides
- Other integrations — SharePoint, Outlook, FTP, etc.
...
{Which module to work on?}
  ├─ krista-whatsapp → [Use Java 21, Gradle, LangChain4J, virtual threads]
  │                      ↓
  │                    [Active development — make changes here]
  │                      ↓
  │                    END
  │
  ├─ Automation SDK → [Use Java 17+, independent codebase]
  │                     ↓
  │                   END
  │
  └─ heloai-waba-* → [STOP — deprecated; do NOT modify]
                       ↓
                     [Redirect work to krista-whatsapp]
                       ↓
                     END
```
...
- api/ — JAX-RS endpoints (WhatsApp webhook + admin)
- orchestrator/ — the brain (state machine, ToolExecutor, tools)
- chat/ + util/ — LangChain4J wiring, prompt builder (InstructionBuilder)
- service/ — HeloAI client, ActiveMQ listener, cleanup schedulers
- persistence/ — Infinispan repositories, Protostream BOs
- catalog/ — exposed Krista catalog requests (Get Transcript, etc.)
- monitoring/ — JFR virtual-thread pinning monitor

**Workflow** — inter-package data flow

```text
START
  ↓
[Inbound webhook lands in api/WhatsAppResource]
  ↓
[Hand off to orchestrator/SessionOrchestrator on virtual thread]
  ↓
[Acquire per-phone lock via service/SessionLockManager]
  ↓
[Load WabaSession via persistence/SessionRepository]
  ↓
{Current orchestration state?}
  ├─ Init        → [chat/LlmService.analyzeInitMessage]
  ├─ Idle        → [orchestrator/ToolExecutor + conversation tools]
  ├─ Collecting  → [orchestrator/ToolExecutor + SubmitAnswer / PickOne / Exit]
  └─ Exit        → [Yes/No confirmation handler]
  ↓
[Tool may call service/PlatformService → Krista Studio]
  ↓
[Outgoing reply → service/HeloAIWabaClient → HeloAI gateway]
  ↓
[Persist staged messages → persistence/SessionMessageRepository]
  ↓
[Release lock]
  ↓
END
...
- HeloAI WABA Gateway — `https://wabaapp.helo.ai`
- Krista Appliance (Docker, Java 21)
  - krista-whatsapp Extension (JAX-RS)
  - Krista Studio Platform SDK (in-process)
- Infinispan cluster (HotRod TCP, SCRAM-SHA-512, port 11222)
- ActiveMQ broker (topic: `chatbot-message-state-notifications`)
- OpenAI API (`gpt-4o-mini`)

**Workflow** — message lifecycle through the deployed system
...
[User sends WhatsApp message]
  ↓
[WhatsApp Business API delivers to HeloAI Gateway]
  ↓
[HeloAI POSTs webhook to Extension]
  ↓
[Extension reads/writes session in Infinispan via HotRod]
  ↓
[Extension calls OpenAI over HTTPS (Bearer token)]
  ↓
{LLM picks a tool?}
  ├─ Yes → [Extension calls Krista Studio (in-process SDK)]
  │         ↓
  │       [Studio runs the chosen workflow]
  │         ↓
  │       [Studio publishes completion event → ActiveMQ topic]
  │         ↓
  │       [Extension's ConversationCompletionListener consumes]
  │         ↓
  │       [Orchestrator resumes and prepares reply]
  │
  └─ No  → [Extension composes the LLM's reply directly]
  ↓
[Extension POSTs reply to HeloAI (Bearer token)]
  ↓
[HeloAI delivers to WhatsApp Business API → user's phone]
  ↓
END
```
...
- User (WhatsApp)
- HeloAI Webhook
- WhatsAppResource (JAX-RS)
- SessionOrchestrator (virtual thread + per-phone lock)
- LLM + ToolExecutor
- Krista Studio (ConversationExecution)
- ActiveMQ topic
...
START
  ↓
[User msg arrives on WhatsApp]
  ↓
[HeloAI webhook → WhatsAppResource]
  ↓
[SessionOrchestrator picks up on virtual thread + acquires per-phone lock]
  ↓
{Orchestration state?}
  │
  ├── InitState
  │     ↓
  │   [Detect language, send greeting]
  │     ↓
  │   [Transition → Idle]
  │     ↓
  │   END (this turn)
  ...
  ├── IdleState
  │     ↓
  │   [LLM sees ALL conversation tools]
  │     ↓
  │   {LLM chose a workflow?}
  │     ├─ Yes → [Launch ConversationExecution in Krista Studio]
  │     │         ↓
  │     │       [Transition → Collecting]
  │     │
  │     └─ No  → [Send free-text reply]
  │
  ├── CollectingFieldState
  │     ↓
  │   [LLM sees SubmitAnswer / PickOne / Exit tools]
  │     ↓
  │   {All required fields collected?}
  │     ├─ Yes → [submit_answer → Krista Studio]
  │     │         ↓
  │     │       [Wait for ActiveMQ completion event]
  │     │         ↓
  │     │       [Send result to user]
  │     │         ↓
  │     │       [Transition → Idle]
  │     │
  │     └─ No  → [Ask user the next field]
  │
  └── ExitState
        ↓
      [Yes/No confirmation]
        ↓
      {User confirmed?}
        ├─ Yes → [Cleanup + capture transcript]
        │         ↓
        │       END
        │
...
- Docker container (baseImage `3.5.7-sp1`)
  - JVM Java 21 (virtual threads enabled)
  - Invoker = krista-whatsapp
  - Jersey JAX-RS
  - HK2 DI
  - JFR pinning monitor
- Log4j2 (console + file appenders)
- Load balancer / Ingress (external)
- Infinispan cluster (external, shared)
- ActiveMQ broker (external, shared)
- OpenAI (external egress)
- HeloAI (external egress)

**Workflow** — request path through the deployment

```text
START
  ↓
[Inbound HTTPS request from HeloAI]
  ↓
[Load balancer / Ingress receives]
  ↓
[Routes to JAX-RS endpoint inside Docker container]
  ↓
[JVM (Java 21) handles on a virtual thread]
  ↓
[Application reaches external systems in parallel as needed:]
  ├─ Infinispan (HotRod TCP, internal network)
  ├─ ActiveMQ (JMS, internal network)
  ├─ OpenAI (HTTPS egress)
  └─ HeloAI (HTTPS egress)
  ↓
[Logs written to console + file via Log4j2]
  ↓
[JFR monitor reports any pinning events]
  ↓
END (response returned to caller)

NOTE: Multiple container replicas can run side-by-side because
Infinispan + ActiveMQ are shared state.
```
...
- User (WhatsApp)
- WhatsApp Business API → HeloAI WABA Gateway
- WhatsAppResource (JAX-RS)
- SessionOrchestrator + SessionFactory + SessionContext
- SessionRepository (Infinispan + Caffeine L1)
- ToolExecutor + LangChain4J + OpenAI (`gpt-4o-mini`)
- ConversationToolRegistry (dynamic tool list)
- PlatformService → Krista Studio (ConversationExecution)
- ActiveMQ topic → ConversationCompletionListener
- HeloAIWabaClient (outbound)

**Workflow** — full "HI" → "Pay Bill" → bill amount delivered

```text
START
  ↓
[User types "HI" on WhatsApp]
  ↓
[WhatsApp Business API → HeloAI → POST /incoming/messages]
  ↓
[WhatsAppResource validates 'to' phone, returns 200 OK immediately]
  ↓
[Hand off to SessionOrchestrator on a virtual thread]
  ↓
[SessionLockManager.lock(phone) — fair ReentrantLock]
  ↓
[SessionFactory.loadOrCreate(phone)]
  ↓
{Session exists?}
  ├─ No  → [PlatformService.getOrCreateSession → Krista Studio creates account+session]
  │         ↓
  │       [Initial state = InitState, save to Infinispan]
  │
  └─ Yes → [Load WabaSession from Infinispan (Caffeine L1 first)]
  ↓
{Current OrchestrationState?}
  │
  ├── InitState (first ever message)
  │     ↓
  │   [LlmService.analyzeInitMessage → tool call to OpenAI]
  │     ↓
  │   [Returns language (en/hi/mr) + isGreeting]
  │     ↓
  │   [Send greeting via HeloAIWabaClient]
  │     ↓
  │   [Transition → IdleState, flush context, release lock]
  │     ↓
  │   END (this turn — wait for next user message)
  │
  ├── IdleState ("Pay bill" message)
  │     ↓
  │   [ConversationToolRegistry.conversationTools() → [Pay_Bill, Check_Status, ...]]
  │     ↓
  │   [ToolExecutor.executeChatModel with system prompt + tools]
  │     ↓
  │   {LLM returns a tool call?}
  │     ├─ Yes (Pay_Bill) → [ToolContext.launchConversation → Krista Studio]
  │     │                     ↓
  │     │                   [Studio sends back Ask message with required fields]
  │     │                     ↓
  │     │                   [Transition → CollectingFieldState]
  │     │                     ↓
  │     │                   [Bot asks first field, e.g. "Please share your consumer number"]
  │     │
  │     └─ No (free text) → [Send LLM text response]
  │
  ├── CollectingFieldState (user replies "1234567890")
  │     ↓
  │   [ToolExecutor runs with SubmitAnswer + PickOne + Exit tools]
  │     ↓
  │   {LLM calls submit_answer?}
  │     ├─ Yes → [SubmitAnswerTool validates field types / ranges / enums]
  │     │         ↓
  │     │       {Validation passes?}
  │     │         ├─ Yes → [ToolContext.submitAnswer → Krista Studio]
  │     │         │         ↓
  │     │         │       [Transition → IdleState]
  │     │         │         ↓
  │     │         │       [Studio runs workflow asynchronously]
  │     │         │         ↓
  │     │         │       [Studio publishes status="inactive" → ActiveMQ]
  │     │         │         ↓
  │     │         │       [ConversationCompletionListener fires]
  │     │         │         ↓
  │     │         │       [orchestrator.conversationCompleted(event)]
  │     │         │         ↓
  │     │         │       [Studio sends Inform message with result]
  │     │         │         ↓
  │     │         │       [Forward to user via HeloAIWabaClient]
  │     │         │
  │     │         └─ No  → [Send error or re-ask field]
  │     │
  │     └─ No (calls show_options) → [PickOneFieldTool sends interactive list, 9/page]
  │
  └── ExitState
        ↓
      {User says Yes?}
        ├─ Yes → [Fire "Get Transcript" event + full cleanup]
        │         ↓
        │       END
        │
        └─ No  → [Restore previous state]
  ↓
[Flush SessionContext: save dirty session + staged messages to Infinispan]
  ↓
...
- SessionCleanupService (scheduled, every 60 seconds)
- SessionRepository (Infinispan)
- SessionLockManager (per-phone fair lock)
- MessageSender (HeloAIWabaClient)
- PlatformService (Krista Studio cleanup)
- ConversationArea (catalog handler — "Get Transcript")

**Workflow** — automatic session timeout & transcript capture

```text
START (every 60 seconds)
  ↓
[SessionCleanupService.triggerCleanup() on virtual thread]
  ↓
[SessionRepository.findAll() → list of all active WabaSessions]
  ↓
{For each session…}
  ↓
{Idle time > 300 seconds (5 min)?}
  │
  ├─ No  → [Skip this session, continue loop]
  │
  └─ Yes → [SessionLockManager.getLock(phone).lock()]
             ↓
           [Re-check idle time after acquiring lock (double-check pattern)]
             ↓
           {Still idle?}
             │
             ├─ No  → [Another message arrived; release lock, skip]
             │
             └─ Yes → [Send timeout message via MessageSender]
                        ↓
                      [eventHandler.handleEvent("Get Transcript", ...)]
                        ↓
                      [ConversationArea captures transcript, status="Session Timeout"]
                        ↓
                      [SessionRepository.delete(session)]
                        ↓
                      [SessionMessageFactory.evictSession(sessionId)]
                        ↓
                      [PlatformService.cleanupUserSession() → terminate stale execution]
                        ↓
                      [Release lock]
  ↓
END (this run)
  ↓
[Scheduler waits 60 seconds, then repeats]
```

**Parallel scheduled task — TranscriptCleanupService**

```text
START (every 24 hours)
  ↓
[Calculate cutoff = now − 20 days]
  ↓
[SessionMessageRepository.deleteOlderThan(cutoff)]
  ↓
END (next run in 24h)
```
...
- 3 entry points
  - `handlePlatformMessage` (Ask / Inform / Answered from Krista SDK)
  - `processMessage` (IncomingMessage from WhatsApp inbox)
  - `conversationCompleted` (CompletionEvent from ActiveMQ)
- SessionLockManager — per-phone fair `ReentrantLock`
- Virtual thread executor
- SessionContext — load/stage/flush
- State router (Init / Idle / Collecting / Exit)
- `flushContext` — atomic persist of dirty session + staged messages
...
START
  ↓
{Which entry point?}
  ├─ handlePlatformMessage (Ask / Inform / Answered)
  ├─ processMessage (user WhatsApp msg)
  └─ conversationCompleted (ActiveMQ event)
  ↓
[Acquire SessionLockManager.lock(phone) — fair ReentrantLock]
  ↓
[Continue on a virtual thread]
  ↓
[Load SessionContext from Infinispan (Caffeine L1 first)]
  ↓
{Route by orchestration state}
  │
  ├─ InitState        → [handleInit: detect language, greet → transition to Idle]
  │
  ├─ IdleState        → [handleIdle: LLM with ALL conversation tools]
  │                       ↓
  │                     {LLM calls a conversation tool?}
  │                       ├─ Yes → [Launch workflow → transition to Collecting]
  │                       └─ No  → [Send free-text reply]
  │
  ├─ CollectingFieldState → [handleCollecting: SubmitAnswer / PickOne / View More]
  │                            ↓
  │                          {submit_answer succeeded?}
  │                            ├─ Yes → [Transition → Idle]
  │                            └─ No  → [Send next-field prompt]
  │
  └─ ExitState        → [handleExit: Yes/No confirmation]
                          ↓
                        {Yes?}
                          ├─ Yes → [Cleanup + fire transcript event]
                          └─ No  → [Restore previous state]
  ↓
[flushContext: save dirty session; persist staged messages in batch]
  ↓
[Release lock]
  ↓
END
...
States are a sealed Java interface — exhaustive at compile time.

- InitState — first message of a new session
- IdleState — free chat, awaiting intent
- CollectingFieldState — actively collecting fields for an Ask message
- ExitState — confirmation pending (wraps the previous state for restore)
...
[InitState]
  ↓
[First message arrives → detect language, send greeting]
  ↓
[IdleState]
  ↓
{What does the user do?}
  │
  ├── Sends a query the LLM maps to a workflow
  │     ↓
  │   [LLM calls a conversation tool]
  │     ↓
  │   [Krista Studio launches workflow, returns required fields]
  │     ↓
  │   [CollectingFieldState]
  │     ↓
  │   {All required fields collected?}
  │     │
  │     ├─ Yes → [submit_answer succeeds]
  │     │         ↓
  │     │       [Back to IdleState]
  │     │
  │     └─ No  → [Stay in CollectingFieldState]
  │                ↓
  │              {"View More" requested for pickOne?}
  │                ├─ Yes → [Increment page, re-render list → stay Collecting]
  │                └─ No  → [Ask next field]
  │
  ├── Sends EXIT keyword
  │     ↓
  │   [ExitState (wraps current state)]
  │     ↓
  │   {User confirms exit?}
  │     ├─ Yes → [Capture transcript, cleanup → terminal]
  │     └─ No  → [Restore previous state]
  │
  └── Sends free text
        ↓
      [LLM responds, stay in IdleState]
  ↓
END (only on ExitState → Yes path)
```
...
- `SessionRepository` (interface)
  - `findByPhoneNumber`, `findBySessionId`, `findByAccountId`
  - `findAll`, `save`, `delete`, `deleteByAccountId`, `deleteBySessionId`
- `SessionRepositoryImpl`
  - Infinispan RemoteCache (HotRod) — distributed source of truth
  - Caffeine L1 caches: by phone, by sessionId, by accountId
  - 10-minute expiry, 1000-entry max each
- `SessionMessageRepository` (interface)
  - `findById`, `findBySessionId`, `findLastNBySessionId`
  - `save`, `addMessages` (batch), `delete`
  - `deleteBySessionId`, `deleteOlderThan`
- `SessionMessageRepositoryImpl`
  - Infinispan BasicCache (no L1 — messages are write-heavy)
...
{Lookup type?}
  │
  ├─ By accountId → [Direct key get: DefaultKey(invokerId, accountId, 1)]
  │                    ↓
  │                  Sub-millisecond, O(1) — preferred path
  │
  └─ By phone OR by sessionId → [Check Caffeine L1 first]
                                  ↓
                                {L1 hit?}
                                  ├─ Yes → [Return from L1]
                                  │
                                  └─ No  → [Ickle query against Infinispan index]
                                             ↓
                                           [Populate L1, return]
  ↓
END
...
START
  ↓
[Update Caffeine L1 immediately (synchronous)]
  ↓
[objectCache.putAsync(key, value) → non-blocking write to Infinispan]
  ↓
{Async future fails?}
  ├─ Yes → [Log error asynchronously, do not block caller]
  └─ No  → [Persist confirmed in background]
  ↓
END
```
...
- SessionOrchestrator — chooses which LLM path to invoke
- InstructionBuilder + Instructions — composable system prompt chunks
- ToolExecutor — main multi-turn LLM + tool loop
- LlmService — single-shot LLM helpers (analyze, classify, extract)
- ConversationToolRegistry — dynamic list of conversation tools
- Tools — SubmitAnswer, PickOneField, ExitSession, ConversationRemoteTool, SwitchLanguage
- ChatLanguageModelProvider — LangChain4J `OpenAiChatModel` singleton
- OpenAI `gpt-4o-mini` (external)
...
START
  ↓
[SessionOrchestrator decides which LLM path to take]
  ↓
{Path?}
  │
  ├── Initial language detection (first message)
  │     ↓
  │   [LlmService.analyzeInitMessage]
  │     ↓
  │   [Build single-tool ChatRequest: analyze_session_start]
  │     ↓
  │   [OpenAiChatModel.chat → tool call with lang + type]
  │     ↓
  │   END (return InitMessageAnalysis)
  │
  ├── Multi-turn conversation (Idle / Collecting)
  │     ↓
  │   [InstructionBuilder composes system prompt chunks:]
  │     - CORE_IDENTITY, LANGUAGE_ENFORCEMENT, SCOPE_RULES,
  │       FUNCTION_USAGE, RESPONSE_STYLE, EXIT_HANDLING,
  │       INTENT_CLASSIFICATION, withFieldInstructions(state)
  │     ↓
  │   [ConversationToolRegistry supplies tool specs for current state]
  │     ↓
  │   [ToolExecutor.executeChatModel — see Tool Loop diagram]
  │     ↓
  │   END (returns finalResponse + exitFlowTriggered)
  │
  └── Simple chat (relay-only Inform message)
        ↓
      [LlmService.executeSimpleChat — no tools, plain reply]
        ↓
      END
```
...
- ToolExecutor — owner of the loop
- ChatLanguageModelProvider — `OpenAiChatModel` instance
- RemoteToolMap — `name → RemoteTool` and `ToolSpecification` list
- ToolContext — bridge between tools and `SessionContext`
- `ToolResult` — sealed: `Continue`, `StopLoop`, `TriggerExitFlow`

**Workflow** — the heart of LLM-driven dialogue

```text
START
  ↓
[Build messages = [SystemMessage] + last 20 session messages]
  ↓
[Filter dangling tool responses (orphans whose tool-call left the window)]
  ↓
[Set iter = 0]
  ↓
LOOP_START:
...
[iter += 1]
  ↓
{iter > 10 (MAX_ITERATIONS)?}
  ├─ Yes → [Throw RuntimeException]
  │          ↓
  │        END (error)
  │
  └─ No  → [Continue]
  ↓
[chatModel.chat(ChatRequest(messages, toolSpecifications))]
  ↓
[Append AiMessage to messages]
  ↓
{AiMessage contains tool calls?}
  │
  ├─ No  → [Set finalResponse = aiMessage.text()]
  │         ↓
  │       [Set responseFound = true]
  │         ↓
  │       Exit LOOP
  │
  └─ Yes → {Multiple submit_answer calls in same turn?}
             ├─ Yes → [Merge arguments into a single submit_answer call]
             └─ No  → [Continue]
             ↓
           [For each tool request:]
             ↓
           [Look up RemoteTool from RemoteToolMap by name]
             ↓
           [tool.execute(toolContext, args)]
             ↓
           {ToolResult type?}
             │
             ├─ Continue        → [Append ToolExecutionResultMessage; continue inner loop]
             │
             ├─ StopLoop        → [Append result; set responseFound = true; exit LOOP]
             │
             └─ TriggerExitFlow → [Set exitFlowTriggered = true; exit LOOP]
  ↓
GOTO LOOP_START (unless responseFound or exitFlowTriggered)
  ↓
LOOP_END:
  ↓
[Stage newly added messages in SessionContext (not yet persisted)]
  ↓
[Return ExecutionResult(finalResponse, exitFlowTriggered)]
  ↓
END
```
...
**Components** — Business Objects (Protostream-serialised in Infinispan)

- `WabaSession` — root entity, one per phone number
  - `sessionId`, `accountId` (PK), `invokerId`, `phoneNumber`
  - `sessionData` — map of session-level KV pairs
  - `orchestrationDataEntry` — embedded wrapper
- `OrchestrationDataEntry` (embedded inside `WabaSession`)
  - `stateJson` — Jackson-serialised polymorphic `OrchestrationState`
- `SessionMessage` — one per chat turn (user / assistant / system / tool result)
  - `id` (PK, ULID — time-sortable), `sessionId`, `tenantId`
  - `timestamp`, `role`, `content`
  - `toolCalls` (list), `toolCallId`, `toolName`
  - `sequenceNumber`
- `ConversationField` — transient field metadata used at request time
  - `name`, `type` (text / number / pickOne / pickList / bool / date / time / location / ...), `attributes`

**Relationship**

```text
WabaSession  ──── 1 : many ────►  SessionMessage
       │
       └── embeds ──►  OrchestrationDataEntry
```

**Workflow** — how the data is navigated for one chat turn

```text
START (need to load conversation history for a user)
  ↓
[Look up WabaSession by accountId (preferred) OR phoneNumber OR sessionId]
  ↓
{Found?}
  │
  ├─ No  → [Create new WabaSession via SessionFactory]
  │         ↓
  │       END (start of session)
  │
  └─ Yes → [Read orchestrationDataEntry.stateJson]
             ↓
           [Deserialise into OrchestrationState (sealed type)]
             ↓
           [Query last 20 SessionMessages by sessionId ORDER BY id DESC]
             ↓
           [Reverse to chronological order]
             ↓
           [Filter dangling tool responses outside the window]
             ↓
           END (ready to feed to the LLM)
```

**Multi-tenancy invariant**

- Every cache key and every Ickle query is scoped by `invokerId`.
- `WabaSession` key: `DefaultKey(invokerId, accountId, VERSION=1)`
- `SessionMessage` key: `DefaultKey(invokerId, messageId, VERSION=1)`
...
**Components** — time-consuming stages per chat turn

- OpenAI LLM call — ~600 ms (dominant)
- HeloAI send-message — ~200 ms
- Infinispan read/write — ~30 ms (sub-ms on Caffeine L1 hit)
- Local CPU (state routing, JSON, validation, locking) — ~20 ms

**Workflow** — where each chat turn spends its time

```text
START (incoming message)
  ↓
[Local CPU: parse, lock, load context]                ~20 ms
  ↓
[Infinispan: load session + last 20 messages]          ~30 ms
   (or sub-millisecond if Caffeine L1 hot)
  ↓
[OpenAI: chat completion with tools]                  ~600 ms  ◄── DOMINANT
  ↓
{Tool call?}
  ├─ Yes → [Execute tool locally]                       few ms
  │          ↓
  │        [GOTO OpenAI step (loop until responseFound, max 10 iter)]
  │
  └─ No  → [Compose reply]
  ↓
[Infinispan: persist staged messages]                  ~30 ms
  ↓
[HeloAI: POST /messages/single]                       ~200 ms
  ↓
END  (total ≈ 850 ms — dominated by external LLM call)
```

**Optimisation levers**

- 20-message context window cap → keeps OpenAI token cost bounded
- Caffeine L1 cache → eliminates Infinispan hop on hot reads
- Virtual threads → keep blocking I/O cheap under load
- `gpt-4o-mini` (smallest capable model) → fastest tier
- No streaming → WhatsApp can't render progressive responses anyway
